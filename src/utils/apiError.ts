...
const translateMessage = (value: unknown, fallback: string): string => {
  ...
  const arabicTranslations: Array<[string, string]> = [
    ...
    ['لا يمكن للمراجع تعديل الطلب بعد اعتماده وإرساله إلى المرحلة التالية', 'تم اعتماد الطلب وإرساله إلى المرحلة التالية، لذلك تم قفل التعديل نهائيًا. يمكنك فتح الطلب للعرض فقط.'],
  ];
